...
//Code Here
let postTaxPrices = prices.map((nums) => nums * 1.07) // = prices.map(/* Provide Your Callback Here );
...
//Code Here
let totalPopulation = populations.reduce((total, items) => {return total + items}) //  = populations.reduce(/* Provide Your Callback Here */)
...
//Code Here
let myStrongest = monstersInYourPocket.filter((value) => value.CP > 200) // = monstersInYourPocket.filter(/* Provide Your Callback Here */)
...
let bobsTotal = purchases.filter((value) => value["owner"] === "Bob").reduce((total, item) => total + item.price, 0)
